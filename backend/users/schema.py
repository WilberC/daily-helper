...
class Mutation:
    ...
    def login(self, info: Info, username: str, password: str) -> LoginPayload:
        ...
        if "@" in username:
            try:
                ...
                return LoginPayload(
                    success=False, message="Invalid username or password", user=None
                )

        # Authenticate the user
        user = authenticate(
            info.context.request, username=actual_username, password=password
        )
        ...
        if user is not None:
            if user.is_active:
                ...
                return LoginPayload(success=True, message="Login successful", user=user)
            else:
                return LoginPayload(
                    success=False, message="This account has been disabled", user=None
                )
        else:
            return LoginPayload(
                success=False, message="Invalid username or password", user=None
    ...
    def register(self, info: Info, input: RegisterInput) -> RegisterPayload:
        ...
        user = User.objects.create_user(
            ...
            is_superuser=input.is_superuser or False,
    ...
    def update_user(self, info: Info, input: UpdateUserInput) -> UpdateUserPayload:
        ...
        try:
            ...
            if user.is_superuser and str(user.id) != str(current_user.id):
                # Allow updating email, first_name, last_name but not permissions
                if (
                    input.is_staff is not None
                    or input.is_superuser is not None
                    or input.is_active is not None
                ):
                    return UpdateUserPayload(
                        success=False,
            ...
            return UpdateUserPayload(
                success=False, message="User not found.", user=None
            )
        except Exception as e:
            return UpdateUserPayload(
                success=False, message=f"Error updating user: {str(e)}", user=None
            )
    ...
    def logout(self, info: Info) -> LogoutPayload:
        ...
        if info.context.request.user.is_authenticated:
            ...
            return LogoutPayload(success=True, message="Logout successful")
        else:
            return LogoutPayload(
                success=False, message="No user is currently logged in"
            )
